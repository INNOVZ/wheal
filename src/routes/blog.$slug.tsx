...
export const Route = createFileRoute("/blog/$slug")({
  head: ({ params }) => {
    ...
    return {
      meta: [
        { title: post ? `${post.title} — Together We Heal` : "Post — Together We Heal" },
        { name: "description", content: post?.excerpt ?? "Reflections on healing." },
  ...
  notFoundComponent: () => (
    <PageShell>
      <div className="px-6 md:px-10 max-w-[1400px] mx-auto py-24">
        ...
        <Link to="/blog" className="font-mono text-xs hover:underline">← Back to blog</Link>
      </div>
    </PageShell>
  ),
  loader: ({ params }) => {
  ...
  return (
    <PageShell>
      <article className="px-6 md:px-10 max-w-[900px] mx-auto pt-12 pb-20">
        <Link to="/blog" className="inline-flex items-center gap-2 font-mono text-[11px] text-foreground/60 hover:text-foreground mb-8">
          <ArrowLeft className="h-3.5 w-3.5" /> BACK TO BLOG
        </Link>
        <div className="font-mono text-[11px] text-foreground/55 mb-4">{post.cat.toUpperCase()}</div>
